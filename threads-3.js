...
// SF: MOD: trigger "first" script of sprite
...
/// world.children[0].sprites.contents[0].scripts.children[0]
    ...
    if (this.homeContext.receiver) {
        stage = this.homeContext.receiver.parentThatIsA(StageMorph);
        if (stage) {
			sprite = stage.sprites.contents.filter(function (sprite) {
				return sprite.isVisible && sprite.name == name;
			});
			if(sprite) {
				firstScriptOfSprite = sprite.scripts.children[0];
				stage.threads.toggleProcess(firstScriptOfSprite);
			}
        }
    }
};
